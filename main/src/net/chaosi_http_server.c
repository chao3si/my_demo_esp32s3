...
static esp_err_t http_resp_dir_html(httpd_req_t *req, const char *dirpath)
{
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        ESP_LOGI(TAG, "the Hyperlinks is %s", req->uri);
...
/**
 * @description: dest是文件完整存储路径，返回的是文件名称部分指针
 * @param {char} *dest          文件完整路径
 * @param {char} *base_path     server_data->base_path
 * @param {char} *uri           响应req的uri
 * @param {size_t} destsize     dest的最大长度
 * @return {const char*}                  文件名称部分指针
 */
...
static esp_err_t download_get_handler(httpd_req_t *req)
{
    ...
    ESP_LOGI(TAG, "download_get_handler================filename is %s", filename);
    ESP_LOGI(TAG, "download_get_handler================filepath is %s", filepath);
    ESP_LOGI(TAG, "download_get_handler================base_path is %s", ((struct file_server_data *)req->user_ctx)->base_path);
...
static esp_err_t send_wifi_handler(httpd_req_t *req)
{
    ...
    {
        httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "content is too long");
        return ESP_FAIL;
    }
    //没接收完的时候持续接收
    while(cur_len < total_len)
    {
        receive = httpd_req_recv(req, buf + cur_len, total_len);
        if(receive <= 0)
        {
            httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Fail to post value");
            return ESP_FAIL;
        }
        cur_len += receive;
    }
    ...
    /*设置sta模式下的ssid和psw,同步到system_data*/
    nvs_write_wifi_info(ssid, psw);
    //printf_wifi_info();
    /*删除cjson对象*/
    cJSON_Delete(root);
    /*发送wifi更新事件*/
    wifi_send_event(WIFI_STA_UPDATE);
    /*发送应答*/
    httpd_resp_sendstr(req, "wifi information is successfully updated");
    return ESP_OK;
}
